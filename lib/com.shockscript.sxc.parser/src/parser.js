...
class Subparser {
	...
	parseOptPrimaryExpression() {
		...
		if (s)
			...
			r = parseStructureInitializer(null);
		}
			...
			if (this.token.type == Token.LBRACE)
				r = this.parseStructureInitializer(base);
	...
	parseStructureInitializer(base) {
		let fields = [];
		this.expect(Token.LBRACE);
		while (this.token.type != Token.RBRACE) {
			fields.push(this.parseStructureInitializerField());
			if (!this.consume(Token.COMMA))
				break;
		}
		...
		let r = null;
		if (base != null) {
			r = new AST.StructOrFlagsLtr(base, fields);
			this.markLocation(base.span);
			this.finishNode(r);
		} else {
			r = new AST.StructOrFlagsLtr(null, fields);
		}
		return r;
	}

	parseStructureInitializerField() {
		this.markLocation();
		let r = null;
		if (this.consume(Token.ELLIPSIS))
			...
			r = new AST.StructLtrField(name, value);
		}
		this.finishNode(r);
		return r;
	}
	...
	parseTryStmt(context) {
		...
		while (this.token.isKeyword('catch')) {
			...
			let startSpan = this.getTokenSpan();
			this.nextToken();
			let c = new AST.TryCatch(this.parseDestructuringPattern(), this.parseBlock(context.clone(), startSpan));
			this.finishNode(c);
			catchClauses.push(c);
		}
	...
	parseSelectTypeStmt(context) {
		...
		while (this.token.isKeyword('case')) {
			...
			if (this.consumeKeyword('else')) {
				...
				c = new AST.SelectTypeCase(this.parseDestructuringPattern(), this.parseBlock(context.clone(), startSpan));
			}
